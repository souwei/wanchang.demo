...
(function () {
    ...
    _Sellect.prototype.addToSelected = function(newSelection) {
      var self = this;

      newSelection.forEach(function(item) {
          var exist = false;
          for (var i = 0; i < self.destinationListHTML.childNodes.length; i++) {
              if (self.destinationListHTML.childNodes[i].textContent === item) {
                  exist = true;
              }
          }

          for (var i = 0; i < self.originListHTML.childNodes.length; i++) {
              if (self.originListHTML.childNodes[i].textContent === item) {
                  self.originListHTML.removeChild(self.originListHTML.childNodes[i])
              }
          }
